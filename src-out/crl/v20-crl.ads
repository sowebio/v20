...
------------------------------------------------------------------------------
--
-- System packages for dynamic and static link only: sudo apt install libcurl4 libcurl4-openssl-dev
--
-- System packages for static link only: sudo apt install libnghttp2-dev libidn2-0 librtmp-dev libidn2-0-dev libidn2-dev libpsl-dev libkrb5-dev libkrb5-3 libgssapi-krb5-2 libldap2-dev
-- /!\ libkrb5-dev.a libkrb5.a et libgssapi-krb5.a in system packages are missing, so static linking is a no go...
--
-- In v20. Gpr, replace Common_Linker_Options := ("-static"); with:
-- Dynamic linking (ok) : Common_Linker_Options := ("-lcurl");
-- Static linking (ko): Common_Linker_Options := ("-static", "-lcurl", "-lnghttp2", "-lidn2", "-lrtmp", "-lpsl", "-lssl", "-lcrypto", "-lssl", "-lcrypto", "-lgssapi_krb5", "-lkrb5", "-lk5crypto", "-lcom_err", "-lldap", "-llber", "-lz");
--
